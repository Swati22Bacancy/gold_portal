...
class WiseTransactions extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($accountarray as $account)
        {
            ...
            {
                $lasttransactions = DB::table('wise_transactions')->where('account_id',$accountid)->latest()->get()->first();
            ...
            foreach ($transactions as $index => $transaction)
            {
                ...
                $existtransaction = DB::table('wise_transactions')->where('account_id',$accountid)->where('transaction_id',$transaction->transactionId)->count();
                
                if($existtransaction==0)
                {   
                    $tran = DB::table('wise_transactions')->insert([
