<template>
  <div>
    <form class="crt-paymentcategory" @submit.prevent="update_paymentcategory">
    <!-- Page Heading -->
    ...
    <div class="row">
      <div class="col-md-12 createpaymentcategory-div">
...
export default {
  ...
  methods:
  {
    async update_paymentcategory() {
       this.$v.formdata.$touch();
      if (this.$v.formdata.$error) {
        return;
      }
      try {
        const response = await axios.post("update_paymentcategory", {
          id: this.$route.params.id,
          name: this.formdata.name,
        });
        let message =
            "Payment Category has been successfully updated.";
  ...
  mounted()
  {
    axios.get('/paymentcategory_details/'+this.$route.params.id)
        .then((response) => {
